...
class ERR_CONFIGSET_NOT_FOUND(Exception):
    pass
...
class ConfigSetModel():
    ...
    def get(self, config_id):
        ...
        if obj == None:
            raise(ERR_CONFIGSET_NOT_FOUND(f"Config set not found: {config_id}"))

        configs = [m for m in obj if re.match('CS\#[A-Za-z0-9\-]+$', m[SORT_KEY])]
        if len(configs) == 0:
            return None
        ...
        rl = [item for item in obj if re.match(f'CR\#[A-Za-z0-9\-]+$', item[SORT_KEY])]
        if len(rl) > 0:
            recipe = rl[0]
        ...
        self.labels = []
        for l in [m for m in obj if re.match(f'CL\#.+$', m[SORT_KEY])]:
